...
<aside class="note" title="Accuracy of totalItems">
  <p>Maintaining an exact count can be expensive or infeasible for very large or rapidly changing containers. The relaxed requirement lets a server return an approximate value in such cases rather than omitting the property or failing the
  request, while still encouraging an exact count wherever it is practical to compute one.</p>
</aside>
